---
title: Popover
slug: /components/popover
group: Overlays
order: 38
description: Popover is a floating panel anchored to a trigger element, with a placement prop that flips when space runs out and an optional internal close button.
---

# Popover

Pass a React element as `trigger` and the panel body as `children`. The machine
manages placement, focus, and dismissal.

<Demo name="popover/basic" center />

## Placement

`placement` anchors the panel to a side of the trigger, optionally aligned to its
start or end edge. It defaults to `bottom`, and the machine flips the panel when
there is not enough room on the requested side.

<Demo name="popover/placements" center />

## Close button

Set `showCloseButton` to add an internal dismiss control.

<Demo name="popover/close-button" center />

## API

<PropsTable component="popover" />

## Styling

### Anatomy

<Anatomy component="popover" />

### Tokens

<TokenTable component="popover" />
