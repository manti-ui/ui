---
title: Listbox
slug: /components/listbox
group: Inputs
order: 31
description: Listbox is a selectable list of value and label options with roving focus, typeahead, single or multiple selectionMode, and sizes shared with Select and Menu.
---

# Listbox

Pass options as `{ value, label, disabled? }`. The machine manages roving focus,
typeahead, and selection.

<Demo name="listbox/basic" center />

## Sizes

`size` sets the row rhythm: type, padding, and icon size. It is the same scale
Select, Combobox, and Menu use, so a list next to a `sm` control reads at the
same size.

<Demo name="listbox/sizes" center />

## Multiple selection

Set `selectionMode="multiple"` and use array values.

<Demo name="listbox/multiple" center />

## API

<PropsTable component="listbox" />

## Styling

### Anatomy

<Anatomy component="listbox" />

### Tokens

<TokenTable component="listbox" />
