...
export interface DataTableRowComponentProps<TData> {
  /** The original consumer row value. */
  row: TData;
  /** Current row-model index after sorting/filtering/pagination. */
  index: number;
  /** Props that must be spread onto the semantic `<tr>`. */
  rowProps: WithDataAttributes<HTMLAttributes<HTMLTableRowElement>>;
  /** Cells generated by the table column model. */
  children: ReactNode;
}
...
export interface DataTableProps<TData> {
  ...
  /** Add native/ARIA/data props to a row. */
  getRowProps?: (
    row: TData,
    index: number,
  ) => WithDataAttributes<HTMLAttributes<HTMLTableRowElement>>;
  /** Add native/ARIA/data props to a cell. */
  getCellProps?: (
    row: TData,
    columnId: string,
    index: number,
  ) => WithDataAttributes<TdHTMLAttributes<HTMLTableCellElement>>;
  /**
   * Optional component boundary for each row. Unlike a render callback, this
   * component may safely call hooks while resolving normalized entity refs.
   */
  rowComponent?: ComponentType<DataTableRowComponentProps<TData>>;
  id?: string;
  className?: string;
}
...
function DefaultRow<TData>({
  rowProps,
  children,
}: DataTableRowComponentProps<TData>) {
          ...
          <tbody data-scope={SCOPE} data-part="body">
            {rows.length === 0 ? (
              ...
              rows.map((row) => {
                ...
                } as WithDataAttributes<HTMLAttributes<HTMLTableRowElement>>;
